...
public:
    ...
    galay::Proto_Judge_Type IsPduAndLegal(const std::string& buffer) override
    {
        if(buffer.length() < sizeof(self_head)) return galay::Proto_Judge_Type::PROTOCOL_INCOMPLETE;
        if(m_header_len == 0){
        ...
        if(buffer.length() < m_head.length + sizeof(self_head)) return galay::Proto_Judge_Type::PROTOCOL_INCOMPLETE;
        return galay::Proto_Judge_Type::PROTOCOL_LEGAL;
    }
...
public:
    ...
    galay::Proto_Judge_Type IsPduAndLegal(const std::string& buffer)
    {
        if(buffer.length() < sizeof(self_head)) return galay::Proto_Judge_Type::PROTOCOL_INCOMPLETE;
        if(m_header_len == 0){
            memcpy(&m_head.version,buffer.substr(0,4).c_str(),sizeof(4));
    ...
    int DecodePdu(const std::string &buffer)
    {
        m_body = buffer.substr(m_header_len,m_head.length);
        int res = m_header_len+ m_head.length;
        m_header_len = 0;
