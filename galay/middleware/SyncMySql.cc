...
galay::MiddleWare::SyncMySql::SyncMySql(std::string charset)
{
    ...
    if(this->m_handle==NULL) {
        spdlog::error("{} {} {} mysql_init error is '{}'" , __TIME__ , __FILE__ , __LINE__ , mysql_error(this->m_handle));
        throw MySqlException(mysql_error(this->m_handle));
    }
    if(mysql_options(this->m_handle,MYSQL_SET_CHARSET_NAME,charset.c_str()) == -1){
        spdlog::error("{} {} {} mysql_options error is '{}'" , __TIME__ , __FILE__ , __LINE__ , mysql_error(this->m_handle));
        throw MySqlException( mysql_error(this->m_handle));
    }
}
...
galay::MiddleWare::SyncMySql::Connect(const std::string& Remote,const std::string& UserName,const std::string& Password,const std::string& DBName, uint16_t Port)
{
    ...
    {
        spdlog::error("{} {} {} {}",__TIME__,__FILE__,__LINE__,mysql_error(this->m_handle));
...
galay::MiddleWare::SyncMySql::CreateTable(std::string TableName, const std::vector<std::tuple<std::string,std::string,std::string>>& Field_Type_Key)
{
    ...
    {
        spdlog::error("{} {} {}  Arg: [TableName] or [Field_Type_Key] empty",__TIME__ , __FILE__,__LINE__);
        return -1;
    }    
    ...
    {
        spdlog::error("{} {} {} mysql_ping error is '{}'",__TIME__ , __FILE__,__LINE__,mysql_error(this->m_handle));
        return -1;
    }
    spdlog::info("{} {} {} mysql query is '{}'",__TIME__ , __FILE__,__LINE__,query);
    int ret = mysql_real_query(this->m_handle,query.c_str(),query.size());
    if(ret){
        spdlog::error("{} {} {} msyql_real_query error is '{}'",__TIME__ , __FILE__,__LINE__,mysql_error(this->m_handle));
        return -1;
    }
    return 0;
}
...
galay::MiddleWare::SyncMySql::Select(const std::string& TableName,const std::vector<std::string> &Fields,const std::string& Cond)
{
    ...
    {
        spdlog::error("{} {} {} Arg: [Fields] or [TableName] empty",__TIME__, __FILE__,__LINE__);
        return {};
    }
    ...
    if(m_res == nullptr) {
        spdlog::error("{} {} {} mysql_store_result error is '{}'",__TIME__ , __FILE__,__LINE__,mysql_error(this->m_handle));
        return {};
    }
...
galay::MiddleWare::SyncMySql::ParamSendBinaryData(const std::string &ParamQuery, const std::string& Data)
{
    ...
    if(stmt == nullptr) {
        spdlog::error("{} {} {} mysql_stmt_init error is '{}'",__TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    ...
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_prepare error is '{}'", __TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    MYSQL_BIND param = {0};
    ...
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_bind_param error is '{}'", __TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    ret = mysql_stmt_send_long_data(stmt, 0, Data.c_str(), Data.length());
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_send_long_data error is '{}'", __TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    ret = mysql_stmt_execute(stmt);
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_execute error is '{}'", __TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    ret = mysql_stmt_close(stmt);
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_close error is '{}'", __TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    return 0;
}
...
galay::MiddleWare::SyncMySql::ParamRecvBinaryData(const std::string &ParamQuery, std::string& Data)
{
    ...
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_bind_result error is '{}'",__TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    ...
    if (ret)
    {
        spdlog::error("{} {} {} mysql_stmt_store_result error is '{}'",__TIME__, __FILE__, __LINE__, mysql_error(this->m_handle));
        return -1;
    }
    char buffer[1024] = {0};
    while (1)
    {
        ...
        if (ret != 0 && ret != MYSQL_DATA_TRUNCATED){
            spdlog::info("{} {} {} mysql_stmt_store_result success",__TIME__, __FILE__, __LINE__);
            break; // 异常
        }
        long long save = 0;
...
galay::MiddleWare::SyncMySql::StartTransaction()
{
    ...
    {
        spdlog::info("File:{} Line:{} {}",__FILE__,__LINE__,mysql_error(this->m_handle));
        return false;
    }
    return true;
}
