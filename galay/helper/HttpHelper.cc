...
bool
HttpFormDataHelper::IsFormData(http::HttpRequest::ptr request)
{
...
bool 
HttpFormDataHelper::ParseFormData(http::HttpRequest::ptr request, std::vector<FormDataValue>& values)
{
    std::string contentType = request->Header()->HeaderPairs().GetValue("Content-Type");
...
void 
HttpFormDataHelper::FormDataToString(http::HttpRequest::ptr request, const std::string& boundary, const std::vector<FormDataValue>& values)
{
...
bool HttpHelper::DefaultGet(http::HttpRequest *request, const std::string &url, bool keepalive)
{
    ...
    request->Header()->Version() = http::HttpVersion::Http_Version_1_1;
    request->Header()->Method() = http::HttpMethod::Http_Method_Get;
...
bool HttpHelper::DefaultRedirect(http::HttpResponse *response, const std::string &url, HttpResponseCode code)
{
    DefaultHttpResponse(response, http::HttpVersion::Http_Version_1_1, code, "text/html", "");
    response->Header()->HeaderPairs().AddHeaderPair("Location", url);
    return true;
}

bool HttpHelper::DefaultHttpResponse(http::HttpResponse *response, http::HttpVersion version, http::HttpStatusCode code, std::string type, const std::string &body)
{
