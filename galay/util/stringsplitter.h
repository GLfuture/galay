...
namespace galay::tools
{
    ...
    public:
        ...
        static std::vector<std::string> SpiltWithCharConnectWithQuote(const std::string &str, const char partition)
        {
            return Spilt_With_Char_Connect_With_char(str, partition, '\'');
        }

        // 用一个字符分割字符串，双引号中的内容视为一个整体，\"进行转移
        static std::vector<std::string> SpiltWithCharConnectWithDoubleQuote(const std::string &str, const char partition)
        {
            return Spilt_With_Char_Connect_With_char(str, partition, '\"');
        }
        ...
        static std::vector<std::string_view> SpiltWithCharConnectWithQuote(std::string_view str, const char partition)
        {
            return Spilt_With_Char_Connect_With_char(str, partition, '\'');
        }
        static std::vector<std::string_view> SpiltWithCharConnectWithDoubleQuote(std::string_view str, const char partition)
        {
            return Spilt_With_Char_Connect_With_char(str, partition, '\"');
        }
#endif
