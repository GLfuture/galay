        ...
        int exec() override
        {
            ...
            case Task_Status::GY_TASK_WRITE:
            {
                if (Tcp_RW_Task<REQ, RESP>::send_package() == -1)
                    return -1;
                if(this->m_wbuffer.empty()) Tcp_RW_Task<REQ, RESP>::control_task_behavior(Task_Status::GY_TASK_DISCONNECT);
                break;
            }
            default:
                break;
        ...
        int exec() override
        {
            ...
            case Task_Status::GY_TASK_WRITE:
            {
                if (Tcp_SSL_RW_Task<REQ, RESP>::send_package() == -1)
                    return -1;
                Tcp_SSL_RW_Task<REQ, RESP>::control_task_behavior(Task_Status::GY_TASK_DISCONNECT);
                break;
            }
            default:
                break;
