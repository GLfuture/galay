...
namespace galay
{
    ...
    public:
        ...
        Http_Server(Http_Server_Config::ptr config, IO_Scheduler<REQ,RESP>::ptr scheduler):
    ...
    protected:
        void add_accept_task(std::function<Task<>(std::shared_ptr<Task_Base<REQ, RESP>>)> &&func , uint32_t max_recv_len) override
        {
            ...
            this->m_scheduler->m_tasks->emplace(std::make_pair(this->m_fd, std::make_shared<Http_Accept_Task<REQ, RESP>>(this->m_fd, this->m_scheduler
                , std::forward<std::function<Task<>(std::shared_ptr<Task_Base<REQ, RESP>>)>>(func), max_recv_len)));
        }
    };
    ...
    public:
        ...
        Https_Server(Https_Server_Config::ptr config , IO_Scheduler<REQ,RESP>::ptr scheduler)
            :Tcp_SSL_Server<REQ,RESP>(config,scheduler)
        {
    ...
    protected:
        ...
        void add_accept_task(std::function<Task<>(std::shared_ptr<Task_Base<REQ, RESP>>)> &&func , uint32_t max_recv_len) override
        {
            ...
            , std::forward<std::function<Task<>(std::shared_ptr<Task_Base<REQ, RESP>>)>>(func), max_recv_len,config->m_ssl_accept_retry,this->m_ctx)));
        }
    };
